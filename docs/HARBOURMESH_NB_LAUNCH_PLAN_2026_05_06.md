...
- Testing: current tests now cover chart source metadata, Signal K mapping, community sounding extraction, local hazard reporting, account auth, device registration, store queue behavior, and crypto helper regressions; they still do not prove navigation safety, hardware ingest, web account UX, browser layout, or security readiness.
...
- No web account UI, fleet/team authorization, account-aware data ownership checks, uncapped/full hydrography vector tile generation, permanent signed artifact object storage, local official-chart rendering/parser path, or multi-step production release approval workflow exists.
