...
- Security: docs no longer claim production readiness; the weak SHA-256 placeholder key derivation, token signing, and password hashing helpers have been replaced with PBKDF2-HMAC-SHA256/HMAC-SHA256 regressions, the API now has invite-gated account registration/login with PBKDF2-HMAC-SHA256 password storage and HMAC-SHA256 signed account sessions, configurable scoped API-key gates for write/device plus review-operator keys that override client-supplied reviewer names, hash-based API key configuration for deployments that should not store plaintext keys, optional short-lived signed review-operator sessions, optional HMAC-SHA256 artifact download signatures, and the web app can save pilot credentials plus mint/use short-lived review sessions and account sessions from local secret stores excluded from data exports. Production secret management, fleet/team identity, and per-user data authorization are still not implemented.
...
- Testing: current tests now cover chart source metadata, Signal K mapping, community sounding extraction, local hazard reporting, account auth, web account session storage, account ownership metadata, device registration, store queue behavior, and crypto helper regressions; they still do not prove navigation safety, hardware ingest, full browser route coverage, or security readiness.

## Current Verification Snapshot

Last light checks:

- `npm run test:run`: passing, 170 web tests.
...
- `npm run build`: section-level code splitting is active; the initial app JS chunk is 286.94 kB / 90.41 kB gzip, the Navigation section chunk is 41.36 kB / 10.17 kB gzip, the Settings section chunk is 59.81 kB / 12.14 kB gzip, the Community section chunk is 85.45 kB / 13.41 kB gzip, the pilot API credential chunk is 10.47 kB / 3.62 kB gzip, the checkbox chunk is 4.24 kB / 1.82 kB gzip, and the prior Vite oversized chunk warning is gone.
...
- Server API auth tests cover missing keys, accepted header keys, accepted Bearer keys, scoped write/review key separation, review-operator key parsing, short-lived signed review-operator sessions, reviewer identity override for audit history, protected device registry reads, and fail-closed production-style config.
...
- Server account contribution-history tests cover `/api/account/community/contributions`, missing-session 401, invalid-session 403, owner-only filtering across soundings/hazards/observations/devices/aggregate releases, protected device-list account scrubbing, and no cross-account leakage in private responses.
...
- Web account contribution-history tests cover stored account-session headers, device counts in response validation, local no-fetch failure when no account session exists, invalid-session clearing, and malformed response rejection.
- Web device registration tests cover account-session ownership headers on Boat Node registration while preserving pilot API keys.
...
- Local API account auth smoke on port 3134: the built API ran with `HARBOURMESH_ACCOUNT_SESSION_SIGNING_KEY`, `HARBOURMESH_ACCOUNT_SESSION_SIGNING_KEY_ID`, and `HARBOURMESH_ACCOUNT_REGISTRATION_INVITE_CODE`; `POST /api/auth/register`, `POST /api/auth/login`, and `GET /api/auth/me` returned 201/201/200, returned no password hash or salt, and the server was stopped with port 3134 clear.
- Local API account ownership smoke on port 3136: the built API ran with account signing, invite, write key, and review-operator key env; account registration returned 201, account-owned sounding intake returned 202, private review queue exposed the expected `ownerAccountId`, review and aggregate release publish succeeded, public aggregate/overlay payloads did not contain account IDs or ownership field names, an invalid account session returned 403, and the server was stopped with port 3136 clear.
...
- Local API device-provenance smoke on port 3138: two accounts registered two Boat Nodes through `X-HarbourMesh-Account-Session` plus scoped write keys, each account uploaded one sounding from its own device, aggregate release publish returned 201, `/api/devices` listed both devices without account owner fields or account IDs, each private account history showed exactly one owned device, and `/api/community/aggregates.geojson` counted both soundings while omitting account IDs and device IDs.
- Browser smoke on port 5190 with a temporary API on port 3138: Settings Account signed in as `owner-device-smoke-20260506@example.com`, loaded `/api/account/community/contributions` with 200, rendered Total 3 / Soundings 1 / Devices 1 / Releases 1 plus recent `owner-boat-node-smoke-1` device activity at 1280x900 and 360x800, checked body/document width equal to viewport width, captured `output/playwright/settings-account-device-contributions-1280.png` plus `output/playwright/settings-account-device-contributions-360.png`, and reported 0 console errors or warnings.
...
- Hardened `/api/community/overlay.geojson` so public overlay features use hashed public IDs and explicitly exclude raw record IDs, vessel IDs, and source-device IDs.
...
- Promoted `track_point` observations into first-class aggregate cell counts, persisted release cells, PostGIS aggregate columns, vector tile metadata, and map popups so shared track density can be useful without exposing raw vessel routes.
...
- Added optional account-session ownership context for Boat Node/device registration, JSONL/PostGIS private device owner metadata, private device counts/recent device activity in account contribution history, and protected device-list scrubbing so account IDs stay out of shared device reads.
...
- Added optional `HARBOURMESH_SESSION_SIGNING_KEY`, `HARBOURMESH_SESSION_SIGNING_KEY_ID`, and `HARBOURMESH_SESSION_TTL_SECONDS` support plus `POST /api/auth/operator-session` so review-scoped pilot keys can mint short-lived signed Bearer sessions for moderation and release review calls.
- Wired the Settings Network tab to request and locally save short-lived review sessions, and updated hazard moderation plus aggregate-release publishing clients to prefer fresh sessions before falling back to stored long-lived API keys.
...
- Wired the Community Map approval checklist into aggregate release publishing so the web app can attach the reference-only, official-chart-free, raw-ID-free, vessel-ID-free approval payload and render the persisted server-side approver after publish.
- Added `/api/community/hazards/artifacts` and `/api/community/hazards/artifacts/:format` so accepted positioned hazards can be downloaded as reference-only GeoJSON, MBTiles, and PMTiles products with checksums, optional HMAC signatures, and no raw hazard IDs, vessel IDs, source-device IDs, or official chart data.
