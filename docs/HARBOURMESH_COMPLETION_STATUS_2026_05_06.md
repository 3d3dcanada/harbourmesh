...
- `app/src/lib/account-contributions.ts`
- `app/src/lib/account-contributions.test.ts`
...
| Browser/mobile verification | Partial | Targeted smoke checks are recorded, including the latest Settings account contribution-history flow; no full route-by-route browser/mobile pass has been run. |
...
2. Extend account-scoped private reads into fleet/team access policies without breaking the existing pilot API key path.
